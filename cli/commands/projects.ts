  ...
  .addCommand(
    new Command('list').description('List all projects').action(async () => {
      ...
      for (const [index, p] of rows.entries()) {
        console.log(
          `${chalk.cyan(`${index + 1}.`)} ${chalk.bold(p.name)} (${chalk.gray(
            p.subdomain
          )})`
        );
        ...
        console.log(
          `   Created: ${chalk.gray(p.createdAt.toLocaleDateString())}`
        );
        console.log('');
      }
    })
    ...
    new Command('create')
      ...
      .action(async () => {
        ...
        if (availableUsers.length === 0) {
          console.log(
            chalk.red('❌ No users found. Please create a user first.')
          );
          return;
        }
        ...
        const answers = await inquirer.prompt([
          ...
          },
          {
            type: 'input',
            name: 'subdomain',
            message: 'Subdomain (e.g. demo):',
            filter: (v) => String(v).toLowerCase().trim(),
            validate: (v) =>
              /^[a-z0-9-]{2,50}$/.test(v) ||
              'Use lowercase letters, digits, dashes',
          },
          {
            type: 'input',
            name: 'description',
        ...
        try {
          ...
          const userEmail = availableUsers.find(
            (u) => u.id === answers.userId
          )?.email;
  ...
  .addCommand(
    new Command('update')
      ...
      .action(async (sub) => {
        ...
        if (!targetSub) {
          const rows = await db
            .select({ name: projects.name, subdomain: projects.subdomain })
            .from(projects)
        ...
        const [proj] = await db
          .select()
          .from(projects)
          .where(eq(projects.subdomain, targetSub!))
          .limit(1);
        if (!proj) {
          console.log(
            chalk.red(`Project with subdomain "${targetSub}" not found.`)
          );
          return;
        }
        const answers = await inquirer.prompt([
        ...
        const { confirm } = await inquirer.prompt([
          {
            type: 'confirm',
            name: 'confirm',
            message: `Are you sure you want to delete project "${proj.name}" (${proj.subdomain})? This will cascade delete boards/posts/comments.`,
            default: false,
          },
        ]);
        if (!confirm) {
