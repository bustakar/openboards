...
export async function getProjectBySubdomain(sub: string) {
  const { db } = getDatabase();
  const [row] = await db.select().from(projects).where(eq(projects.subdomain, sub)).limit(1);
  return row ?? null;
}

export async function createProject(params: { name: string; subdomain: string; description?: string | null }) {
  const { db } = getDatabase();
  const [row] = await db
    .insert(projects)
    .values({ name: params.name, subdomain: params.subdomain, description: params.description ?? null })
    .returning();
  return row;
}
...
export async function updateProject(
  ...
  const { db } = getDatabase();
  const [row] = await db.update(projects).set(params).where(eq(projects.id, id)).returning();
  return row ?? null;
}
